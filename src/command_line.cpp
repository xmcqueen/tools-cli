...
CommandLine::CommandLine(std::unique_ptr<jinja2::Template> tpl)
    : m_tpl(std::move(tpl)) {}
...
std::string CommandLine::Help() {
  return "Usage: j2cli [options] <input template> <input data>";
}

std::string CommandLine::Version() { return J2CLI_VERSION_STR; }
...
void CommandLine::RenderTemplate(const std::string &fileName) {
  ...
  jinja2::ValuesMap vm = resolver.ParamsFromFile("sample.json");
